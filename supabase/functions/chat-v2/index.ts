...
serve(async (req) => {
  ...
  try {
    ...
    // Merge System Prompt into the first user message (gemini-pro compatibility)
    if (contents.length > 0 && contents[0].role === 'user') {
      contents[0].parts[0].text = `[INSTRUÇÕES DO SISTEMA]: ${finalSystemPrompt}\n\n[MENSAGEM DO USUÁRIO]: ${contents[0].parts[0].text}`;
    } else {
      contents.unshift({
        role: 'user',
        parts: [{ text: `[INSTRUÇÕES DO SISTEMA]: ${finalSystemPrompt}` }]
      });
    }
...
if (!response.ok) {
  ...
  return new Response(JSON.stringify({ error: `Erro Google (${response.status}): ${errorText}` }), {
    status: 500,
    headers: { ...corsHeaders, "Content-Type": "application/json" },
  });
}
