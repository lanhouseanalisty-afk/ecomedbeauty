...
CREATE TYPE marketing_kit_type AS ENUM ('workshop', 'evento', 'treinamento');
CREATE TABLE IF NOT EXISTS marketing_requests (
  ...
  kit_type marketing_kit_type NOT NULL,
  ...
  status marketing_status DEFAULT 'pending',
...
CREATE POLICY "Marketing managers can view all" ON marketing_requests FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM user_roles
...
CREATE POLICY "Marketing managers can update" ON marketing_requests FOR
UPDATE USING (
    EXISTS (
      SELECT 1
      FROM user_roles
...
CREATE POLICY "Approvers can manage assigned requests" ON marketing_requests FOR ALL USING (auth.uid() = approver_id);
...
CREATE INDEX idx_marketing_requests_approver_id ON marketing_requests(approver_id);
-- Create updated_at trigger
CREATE OR REPLACE FUNCTION update_marketing_requests_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER marketing_requests_updated_at BEFORE
UPDATE ON marketing_requests FOR EACH ROW EXECUTE FUNCTION update_marketing_requests_updated_at();