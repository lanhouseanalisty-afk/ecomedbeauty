  ...
  return (
    <>
      {content.announcement.enabled && (
        <div
          className="w-full py-2 px-4 transition-all animate-in fade-in slide-in-from-top duration-500 overflow-hidden"
          style={{ backgroundColor: content.announcement.bg, color: content.announcement.color }}
        >
          <div className="mx-auto max-w-[1400px] text-center">
            <CMSText path="announcement.text" className="text-[10px] font-bold uppercase tracking-[0.3em]" />
          </div>
        </div>
      )}
      <header
        ...
        <nav className="mx-auto flex w-full max-w-[1400px] items-center justify-between px-4 py-4 lg:px-8">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="hidden lg:flex items-center gap-x-10">
              {[
                { label: content.header.menuProducts, path: "/produtos", cmsPath: "header.menuProducts" },
                { label: content.header.menuAbout, path: "/sobre", cmsPath: "header.menuAbout" },
              ].map(item => (
                <Link
                  key={item.label}
                  to={item.path}
                  ...
                  className={`text-[11px] font-bold transition-all cursor-pointer text-center tracking-[0.2em] uppercase
                          ...
                          `}
                >
                  <CMSText path={item.cmsPath as any} />
                </Link>
              ))}
            </div>
          </div>
            ...
            <div className="flex flex-col gap-6 mt-8">
              {[
                ...
                <Link
                  ...
                  onClick={(e) => {
                    handleLinkClick(e);
                    if (!isEditing) setMobileMenuOpen(false);
                  }}
                >
                  {item.label}
                </Link>
