  ...
  return (
    <section className="relative min-h-[750px] w-full overflow-hidden bg-[#2B0F54]">
      ...
      <CMSImage
        ...
        mode="background"
        containerClassName="absolute inset-0 z-0 scale-105 animate-spin-slow opacity-40 blur-[100px] transition-all duration-1000"
      />

      <CMSImage
        path="home.hero.image"
        mode="background"
        containerClassName="absolute inset-0 z-0 transition-all duration-1000"
      >
        <div
          className="absolute inset-0"
          style={{
            backgroundImage: `linear-gradient(to right, #2B0F54 20%, transparent 100%), url('${content.home.hero.image}')`,
      ...
      <div className="relative z-10 mx-auto flex h-full min-h-[750px] max-w-7xl items-center px-4 lg:px-8">
        <div className="max-w-3xl space-y-8 py-20">
          ...
          <h1 className="animate-fade-in-up font-serif text-6xl font-bold leading-[1.1] text-white md:text-8xl">
            <CMSText path="home.hero.titleStart" /> <br />
            <span className="text-gradient">
              <CMSText path="home.hero.titleEnd" />
            </span>
            <span className="text-[#ECB546]">.</span>
          </h1>

          <p className="max-w-xl animate-fade-in-up text-lg leading-relaxed text-white/70 stagger-1">
            <CMSText path="home.hero.subtitle" multiline />
          </p>
          ...
          <div className="flex flex-wrap items-center gap-6 pt-4 animate-fade-in-up stagger-2">
            {isEditing ? (
              <Button
                size="lg"
                className="group h-16 rounded-full bg-[#ECB546] px-10 text-sm font-bold uppercase tracking-widest text-[#2B0F54] transition-all"
              >
                <CMSText path="home.hero.buttonPrimary" />
            ...
            {isEditing ? (
              <div className="flex items-center gap-3 text-white/80 transition-colors group">
                ...
                <span className="text-sm font-semibold tracking-wide">
                  <CMSText path="home.hero.buttonSecondary" />
                </span>
              </div>
            ) : (
              <Link to="/sobre" className="flex items-center gap-3 text-white/80 hover:text-white transition-colors group">
                <div className="flex h-12 w-12 items-center justify-center rounded-full border border-white/20 bg-white/5 transition-colors group-hover:border-[#ECB546]/50">
                  <Beaker className="h-5 w-5" />
                </div>
                <span className="text-sm font-semibold tracking-wide">
