...
try {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // Better parser for "value1","value2" format
        // Simple split by "," and removing quotes valid for this specific file format seen in view_file

        const vals = line.split('","').map(v => v.replace(/^"|"$/g, '').trim());

        // If split failed to produce enough columns or produced too many due to commas, fallback?
        // Let's rely on the simple structure seen: "val","val","val"
        // If line starts with " and ends with ", we can strip them and split by ","
